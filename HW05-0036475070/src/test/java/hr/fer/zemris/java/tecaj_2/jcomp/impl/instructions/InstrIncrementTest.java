...
public class InstrIncrementTest {
	...
	/**
	 * Metoda provjerava broj pozivanja i pristupanja registrima u instrukciji increment.
	 */
	@Test
	public void InstrIncrementRegisterTest() {
		...
		Mockito.when(r.getRegisterValue(0)).thenReturn(new Integer(158));
		
		//kreiraj novu instrukciju s definiranim argumentima
		Instruction instruction = new InstrIncrement(Arrays.asList(arg1));
		
		//izvedi instrukciju
		instruction.execute(c);
		
		//provjeri je li se zatrazilo skup registara dvaput - citanje i upis uvecane vrijednosti
		...
		Mockito.verify(r).setRegisterValue(0, new Integer(159));
	}
