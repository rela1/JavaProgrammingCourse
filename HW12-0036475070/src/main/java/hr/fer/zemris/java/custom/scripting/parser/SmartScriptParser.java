...
/**
 * Klasa za instanciranje objekta koji sluzi za parsiranje dokumenata zeljene
 * strukture.
 * 
 * @author Ivan Relic
 * 
...
public class SmartScriptParser {
	...
	// za appendanje jer se koristi appendanje svakog pojedinog znaka,
	// optimizacije radi
	private StringBuilder buffer;

	/**
	 * Konstruktor prima string koji zelimo parsirati i prosljeduje ga metodi za
	 * parsiranje.
	 * 
	 * @param docBody
	 *            String koji zelimo parsirati.
	 */
	...
	private void parseDocument(String docBody) {
		...
		// boolean varijable za provjeru je li tag escapean ili nije dobro
		// zatvoren
		...
		for (int i = 0; i < duljina; i++) {
			...
			if ((docBodyArray[i] == '{') && !tagEscaped) {
				...
				// ako si dosao do kraja datoteke, a tag je otvoren, baci
				// exception
				if (i == duljina - 1) {
				...
				for (; i < duljina; i++) {
					...
					// ako tag nije pravilno zavrsen, tag je nepravilno zadan i
					// baci exception
					...
					// kraj taga, sastavi ga u string, posalji na provjeru i
					// dodavanje u stablo
					if ((docBodyArray[i] == '}')
							&& (docBodyArray[i - 1] == '$')) {
						String tag = "";
						...
						/*
						 * kad se vratis s provjere taga, postavi da tag nije
						 * otvoren i kreni dalje u parsiranje teksta
						 */
						tagOpened = false;
						break;
					}
			...
			// ako nisi na kraju dokumenta i trenutni znak je \\, a sljedeci {,
			// escapeaj tag
			if (i < duljina - 1) {
		...
		// ako je neki tag ostao otvoren a prosao si cijeli dokument, baci
		// exception
		if (tagOpened) {
			throw new SmartScriptParserException("Bad tag identifier!");
		}
		...
		/*
		 * nakon zavrsene obrade, provjeri ima li na stogu 1 element, ako nema,
		 * znaci da postoji visak/manjak FOR/END naredbi, baci exception
		 */
		if (stack.size() != 1) {
			throw new SmartScriptParserException(
					"Too many FOR tags/too few END tags!");
		}

	}

	/**
	 * Metoda koja tekst iz string buildera ubacuje na stog kao dijete trenutnog
	 * elementa na vrhu.
	 * 
	 */
	...
	/**
	 * Dobiva moguceg reprezenta taga i provjerava je li on uistinu to i obavlja
	 * funkcije koje je potrebno za svaki pojedini.
	 * 
	 * @param tag
	 *            Prima string.
	 */
	private void tagCheck(String tag) {
		...
		/*
		 * ako je tag valjana FOR naredba, oblika varijabla token token (token),
		 * zadnji ne treba biti naveden
		 */
		if (tag.matches("\\{\\$(\\s)*[fF][oO][rR](\\s)*.*(\\s)*.*(\\s)*.*(\\s)*\\$\\}")) {

			// ocisti tag od prva dva, zadnja dva znaka ({$, $}) i kljucne
			// rijeci
			...
			List<Token> kolekcija = new ArrayList<Token>();

			parseTag(tag, kolekcija);

			ForLoopNode forNaredba;

			// ako je u kolekciji samo 3 tokena, pozovi takav konstruktor za
			// ForLoopNode
			if (kolekcija.size() == 3) {
		...
		// ako je tag valjana END naredba
			...
			try {
				...
				throw new SmartScriptParserException(
						"Too many END tags/too few FOR tags!");
			}
		}

		// ako je tag = naredba
		else if (tag.matches("\\{\\$(\\s)*=((.)|(\\s))*(\\s)*\\$\\}")) {
			...
			/*
			 * kad si zavrsio s obradom, spakiraj tokene u niz Token[],
			 * instanciraj EchoNode i posalji na stablo
			 */
	...
	/**
	 * Uzima string koji predstavlja tag i sprema sve tokene u kolekciju koja je
	 * predana kao argument.
	 * 
	 * @param tag
	 *            String koji predstavlja "unutrasnjost" taga, bez {$ i $}
	 * @param kolekcija
	 *            Kolekcija za spremanje tokena.
	 */
	private void parseTag(String tag, List<Token> kolekcija) {
		Scanner scanner = new Scanner(tag);

		// pomocni string u koji ucitavamo argumente naredbe {$= redom i
		// provjeravamo
		String argument;
		...
		while (scanner.hasNext()) {
			...
			// ako je procitan string varijabla, pohrani je u TokenVariable pa
			// kolekciju
			...
			/*
			 * ako je procitani string prikaz stringa, pohrani je u TokenString
			 * pa kolekciju
			 */
				...
				TokenString string = new TokenString(argument.substring(1,
						argument.length() - 1));
				kolekcija.add(string);
			}

			// ako je procitani string operator, pohrani ga u TokenOperator pa
			// kolekciju
			...
			/*
			 * ako je procitani string double broj, pohrani ga u
			 * TokenConstantDouble pa kolekciju
			 */
			else if (isDouble(argument)) {
				TokenConstantDouble brojDouble = new TokenConstantDouble(
						Double.parseDouble(argument));
				kolekcija.add(brojDouble);
			}

			/*
			 * ako je procitani string integer broj, pohrani ga u
			 * TokenConstantInteger pa kolekciju
			 */
			else if (isInteger(argument)) {
				TokenConstantInteger brojInteger = new TokenConstantInteger(
						Integer.parseInt(argument));
				kolekcija.add(brojInteger);
			}

			// ako je procitani string funkcija, pohrani je u TokenFunction pa
			// kolekciju
			else if (isFunction(argument)) {
				TokenFunction funkcija = new TokenFunction(
	 ...
	 * 
	 * @param argument
	 *            Argument za koji provjeravamo je li valjani prikaz funkcije.
	 ...
	 * 
	 * @param argument
	 *            Argument za koji provjeravamo je li valjani integer.
	 * @return True ili false, ovisno o provjeri.
	 */
	 ...
	 * 
	 * @param argument
	 *            Argument za koji provjeravamo je li valjani double.
	 * @return True ili false, ovisno o provjeri.
	 */
	 ...
	 * 
	 * @param argument
	 *            Argument za koji provjeravamo je li valjani operator.
	 * @return True ili false, ovisno o provjeri.
	 */
	 ...
	 * 
	 * @param argument
	 *            Pocetak stringa iz taga.
	 * @param scanner
	 *            Scanner kojim citamo u trenutnom tagu.
	...
	private String readString(String argument, Scanner scanner) {
		...
		if (!argument.matches("(\".*\")")) {
			...
			while (scanner.hasNext()) {
				...
				// inace, samo nadodaj to sto si procitao jer nisi dosao do
				// kraja
				else {
					argument = argument + " " + pom;
				}
