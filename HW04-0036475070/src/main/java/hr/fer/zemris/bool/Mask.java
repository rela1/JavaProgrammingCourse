...
public class Mask {
    ...
    /**Javni konstruktor koji prima MaskValue vrijednosti i pohranjuje ih u svoj container.
     * 
     * @param value Niz MaskValue vrijednosti.
     */
    public Mask(MaskValue ... value) {
	
	//ako je primljena null referenca, baci exception
	if (value == null) {
	    throw new IllegalArgumentException("MaskValue array should not be null!");
	}
	
	//alociraj niz MaskValue velicine broja primljenih argumenata i prekopiraj ih
	this.value = new MaskValue[value.length];
	for(int i = 0, length = value.length; i < length; i++) {
	    
	    //ako je bilo koja od vrijednosti polja null, baci exception
	    if (value[i] == null) {
		throw new IllegalArgumentException("MaskValue array shouldn't contain null!");
	    }
    ...
    /**Metoda prima string koji je reprezentacija maske i vraca novi objekt tipa Mask.
     * 
     * @param maskAsString String iz kojeg zelimo isparsirati vrijednost maske.
     * @return Vraca novi objekt tipa Mask.
     */
    public static Mask parse(String maskAsString) {
	...
	//ako primljeni argument nije oblika jednog ili vise znakova 0,1,x, baci exception
	if (!maskAsString.matches("[01x]+")) {
	    throw new IllegalArgumentException("Mask must contain one or more 0, 1 or x!");
	}
	
	//kreiraj niz MaskValue velicine duljine stringa
	MaskValue[] maska = new MaskValue[maskAsString.length()];
	
	//prodji kroz clanove primljenog stringa i postavi MaskValue na trazenu vrijednost
	...
	return new Mask(maska);
    }
    
    
    /**Metoda prima broj varijabli i mintermu i iz toga kreira novu masku.
     * 
     * @param size Velicina, tj. broj varijabli od kojih se maska sastoji.
     * @param index Minterm koji predstavlja zeljenu masku.
	...
	if (index > Math.pow(2, size) - 1) {
	    throw new IllegalArgumentException("Index should be smaller than 2^size!");
	}
	...
	while (i < size) {
	    ...
	    //trenutna potencija broja 2, krecemo od pocetka i smanjujemo prema kraju
	    int trenPotencija = (int) Math.pow(2, size - 1 - i);
	    
	    /*ako mozes oduzeti potenciju broja dva na trenutnu poziciju i ako vrijednost 
	    indexa nije 0, postavi MaskValue na 1 */
    ...
    /**Metoda koja stapa maske koje se razlikuju u tocno jednoj varijabli u jednu u novu
     * u kojoj je na mjestu te varijable don't care vrijednost.
     * 
     * @param maska1 Prva maska za provjeru, treba biti razlicita od null.
     * @param maska2 Druga maska za provjeru, treba biti razlicita od null.
	...
	if (maska1.value.length != maska2.value.length) {
	    throw new IllegalArgumentException("Masks should be equal in length!");
	}
	
	//kreiraj novo polje MaskValue duljine iste kao primljene maske 
	MaskValue[] novaMaska = new MaskValue[maska1.value.length];
	
	/*prodji kroz obe maske, ako se razlikuju u samo jednome mjestu, vrati kombiniranu
	masku */
	int brojStapanja = 0;
	for (int i = 0, length = maska1.value.length; i < length; i++) {
	    
	    //ako su vrijednosti na i-tom mjestu razlicite stopi ih u DONT CARE
	    if (maska1.value[i] != maska2.value[i]) {
		
		//ako je neka od vrijednosti DONT CARE, vrati NULL
		if (maska1.value[i] == MaskValue.DONT_CARE 
			|| maska2.value[i] == MaskValue.DONT_CARE) {
		    return null;
		}
		
		//inace, vrijednosti su 0 i 1 pa ih stopi
		novaMaska[i] = MaskValue.DONT_CARE;
		brojStapanja++;
	    }
	    
	    //inace, samo prepisi vrijednost iz bilo koje maske
	    else {
		novaMaska[i] = maska1.value[i];
	    }
	}
	
	/* ako je broj stapanja razlicit od jedan, znaci da su na vise od 1 mjesta postojale
	razlicite vrijednosti maske, sto nije dopusteno, ili nije bilo spajanja uopce,
	u tom slucaju vrati null */
	if (brojStapanja != 1) {
	    return null;
	}
	
	//inace, vrati novu kombiniranu masku ako je bilo tocno 1 stapanje
	else {
	    return new Mask(novaMaska);
	}
    }
    ...
    /**Provjerava je li maska nad kojom se zove metoda "generalnija" od maske koja je 
     * primljena kao argument.
     * 
     * @param maska Maska s kojom zelimo provjeriti je li trenutni objekt generalniji od nje.
	...
	if (maska == null) {
	    throw new IllegalArgumentException("Mask should not be null reference!");
	}
	...
	/*prodji kroz obe maske i provjeri podudaraju li im se vrijednosti, ne gledajuci
	dont care vrijednosti */
	for(int i = 0, length = maska.value.length; i < length; i++) {
	    if(this.value[i] != maska.value[i] && !(this.value[i] == MaskValue.DONT_CARE)) {
		return false;
	    }
	}
	
	//ako maska this ima vise DONT CARE vrijednosti, generalnija je
	if (this.value.length - this.getNumberOfOnes() - this.getNumberOfZeroes() 
		> maska.value.length - maska.getNumberOfOnes() - maska.getNumberOfZeroes()) {
	    return true;
	}
     ...
     * 
     * @param index Indeks s kojeg zelimo vrijednost MaskValuea.
     * @return Vrijednost MaskValuea.
     */
    public MaskValue getValue(int index) {
	
	//ako je privatni clanski niz MaskValuea null ili je nedozvoljen indeks, baci exception
	if (this.value == null || index < 0 || index > this.value.length - 1) {
	    throw new IllegalArgumentException("Can't reach element from that index!");
	}
	...
	//dereferenciraj ojekt na klasu Mask i usporedi im polje value, ako nisu isti - false
	Mask other = (Mask) obj;
